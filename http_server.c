...
char *url_decode(const char *url_encoded);
...
char *url_decode(const char *url_encoded) {
    char *decoded = (char *)malloc(strlen(url_encoded) + 1);
    int i = 0, j = 0;
    
    while(url_encoded[i]) {
        if(url_encoded[i] == '%' && i + 2 < strlen(url_encoded)) {
            int value;
            sscanf(&url_encoded[i + 1], "%2x", &value);
